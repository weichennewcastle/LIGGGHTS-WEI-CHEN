...
struct RegisterGranularStylesOMP {
public:
  RegisterGranularStylesOMP() {
    ...
    pair_factory.addVariantSelector("gran/omp", ContactModels::Factory::select);
    wall_factory.addVariantSelector("wall/gran/omp", ContactModels::Factory::select);

    // register granular pair styles
    #define GRAN_MODEL(MODEL,TANGENTIAL,COHESION,ROLLING,SURFACE) \
    registerPair<MODEL, TANGENTIAL, COHESION, ROLLING, SURFACE>("gran/omp", pair_factory); \
    registerWall<MODEL, TANGENTIAL, COHESION, ROLLING, SURFACE>("wall/gran/omp", wall_factory);
...
private:
  ...
  void registerPair(const std::string & name, PairStyles::Factory & factory) {
     typedef GranStyle<MODEL,TANGENTIAL,COHESION,ROLLING,SURFACE> Style;
     typedef ContactModel<Style> CModel;
     typedef PairStyles::GranularOMP<CModel> Type;
